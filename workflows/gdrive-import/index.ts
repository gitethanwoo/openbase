 ...
 *
 * Fetches a Drive file, converts it to text, chunks, embeds, and stores.
 */

import {
  type GDriveImportInput,
  updateSourceStatus,
  fetchGDriveFileText,
  chunkText,
  generateEmbeddings,
...
export async function gdriveImportWorkflow(input: GDriveImportInput) {
  "use workflow";
  ...
  try {
    ...
    const textContent = await fetchGDriveFileText(
      input.fileId,
      input.mimeType,
      input.workosUserId
    );
    ...
    if (!textContent || textContent.trim().length === 0) {
      const errorMsg = "No text content extracted from Google Drive file";
    ...
    if (chunks.length === 0) {
      const errorMsg = "No chunks could be created from Drive content";
      await updateSourceStatus(input.sourceId, "error", errorMsg);
      if (input.jobId) {
        await failJob(input.jobId, errorMsg);
      }
